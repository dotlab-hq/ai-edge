...
export class SkillsProxy {
  ...
  private setupRoutes(): void {
    ...
    this.app.delete("/v1/skills/:skill_id", (c: Context) =>
      this.deleteSkill(c),
    );
    ...
    // ─── Skill Versions ───────────────────────────────
    this.app.get("/v1/skills/:skill_id/versions", (c: Context) =>
      this.listSkillVersions(c),
    );
    this.app.post("/v1/skills/:skill_id/versions", (c: Context) =>
      this.createSkillVersion(c),
    );
    this.app.get("/v1/skills/:skill_id/versions/:version", (c: Context) =>
      this.getSkillVersion(c),
    );
    this.app.delete("/v1/skills/:skill_id/versions/:version", (c: Context) =>
      this.deleteSkillVersion(c),
    );
    this.app.get(
      "/v1/skills/:skill_id/versions/:version/content",
      (c: Context) => this.getSkillVersionContent(c),
    );
    this.app.get("/skills/:skill_id/versions", (c: Context) =>
      this.listSkillVersions(c),
    );
    this.app.post("/skills/:skill_id/versions", (c: Context) =>
      this.createSkillVersion(c),
    );
    this.app.get("/skills/:skill_id/versions/:version", (c: Context) =>
      this.getSkillVersion(c),
    );
    this.app.delete("/skills/:skill_id/versions/:version", (c: Context) =>
      this.deleteSkillVersion(c),
    );
    this.app.get("/skills/:skill_id/versions/:version/content", (c: Context) =>
      this.getSkillVersionContent(c),
    );
    ...
    this.app.get("/v1/files/:file_id/content", (c: Context) =>
      this.downloadFile(c),
    );
    ...
    this.app.get("/files/:file_id/content", (c: Context) =>
      this.downloadFile(c),
    );
  ...
  private async listSkillVersions(c: Context) {
    try {
      ...
      const limit = Number(c.req.query("limit")) || undefined;
      const page = c.req.query("page") || undefined;
      const result = await this.skillStore.listSkillVersions(skillId, {
        limit,
        page,
      });
      return c.json(result);
      ...
      console.error(
        "[/v1/skills/versions] listSkillVersions error:",
        error?.message || String(error),
      );
      return c.json(
        {
  ...
  private async createSkillVersion(c: Context) {
    try {
      ...
      console.error(
        "[/v1/skills/versions] createSkillVersion error:",
        error?.message || String(error),
      );
      return c.json(
        {
    ...
    try {
      ...
      if (!record) {
        return c.json(
          {
            error: {
              message: `Skill version not found: ${skillId}@${version}`,
              type: "not_found_error",
            },
          },
          404,
      ...
      console.error(
        "[/v1/skills/versions] getSkillVersion error:",
        error?.message || String(error),
      );
      return c.json(
        {
  ...
  private async deleteSkillVersion(c: Context) {
    try {
      ...
      const skillId = c.req.param("skill_id") as string;
      const version = c.req.param("version") as string;
      const deleted = await this.skillStore.deleteSkillVersion(
        skillId,
        version,
      );
      if (!deleted) {
      ...
      console.error(
        "[/v1/skills/versions] deleteSkillVersion error:",
        error?.message || String(error),
      );
      return c.json(
        {
  ...
  private async getSkillVersionContent(c: Context) {
    try {
      ...
      const skillId = c.req.param("skill_id") as string;
      const version = c.req.param("version") as string;
      const content = await this.skillStore.getSkillVersionContent(
        skillId,
        version,
      );
      if (!content) {
        return c.json(
          {
            error: {
              message: `Content not found: ${skillId}@${version}`,
              type: "not_found_error",
            },
          },
          404,
      ...
      c.header(
        "Content-Disposition",
        `attachment; filename="skill-${skillId}-v${version}.zip"`,
      );
      return c.body(new Uint8Array(content));
    } catch (error: any) {
      console.error(
        "[/v1/skills/versions/content] getSkillVersionContent error:",
        error?.message || String(error),
      );
      return c.json(
        {
  ...
  private async listFiles(c: Context) {
    try {
      ...
      const result = await this.fileStore.listFiles({
        limit,
        after_id,
        before_id,
        scope_id,
  ...
  private async uploadFile(c: Context) {
    try {
      ...
      if (contentType.includes("multipart/form-data")) {
        ...
        const downloadable =
          formData.get("downloadable")?.toString() === "true";
  ...
  private async downloadFile(c: Context) {
    try {
      ...
      c.header(
        "Content-Disposition",
        `attachment; filename="${fileContent.filename}"`,
      );
      return c.body(new Uint8Array(fileContent.body));
    } catch (error: any) {
      console.error(
        "[/v1/files/content] downloadFile error:",
        error?.message || String(error),
      );
      return c.json(
        {
